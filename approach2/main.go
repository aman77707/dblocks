...
func startBooking(db *sql.DB, user User) {
	...
	err = tx.QueryRow(`SELECT booking_id, seatnumber, user_id FROM bookings WHERE user_id IS NULL ORDER BY booking_id LIMIT 1 FOR UPDATE SKIP LOCKED`).Scan(&bookingid, &seatnumber, &userid)
	if err != nil {
